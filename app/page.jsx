...
// Seed data (used as fallback if DB is empty or errors)
const SEED = [
...
const rowToJob = (row) => ({
  ...
  jobType:   row.job_type  ?? "Kitchen",
  material:  row.material  ?? "Quartz",
  ...
  createdAt: row.created_at ?? TODAYSTR,
});
...
<div class="wrap">
  ...
  <div class="footer">${qn} &middot; CounterTop Pro &middot; Quote valid 30 days from ${shortDate(TODAYSTR)}</div>
</div>
...
<div class="footer">CounterTop Pro &middot; Pipeline Report &middot; ${monthLabel}</div>
<script>window.onload=function(){setTimeout(function(){window.print();},400);}</script>
</body></html>`;
...
<div class="body">
  <div class="total-bar"><span>All-Time Total &middot; ${jobsList.length} job${jobsList.length!==1?"s":""}</span><strong>${fmt(total)}</strong></div>
  <table><thead><tr><th>Type</th><th>Material</th><th>Status</th><th>Quote</th><th>Sq Ft</th><th>Close</th><th>Notes</th></tr></thead>
  <tbody>${rows}</tbody></table>
</div>
<div class="footer">CounterTop Pro &middot; Customer Report &middot; ${name} &middot; ${shortDate(TODAYSTR)}</div>
<script>window.onload=function(){setTimeout(function(){window.print();},400);}</script>
</body></html>`;
  const w = window.open("", "_blank", "width=920,height=1060");
  if (!w) { alert("Allow pop-ups to export PDF."); return; }
...
export default function CountertopCRM() {
  ...
  const fetchJobs = async () => {
    ...
    if (error) {
      ...
      setJobs(SEED); // fallback so UI is never blank
    } else {
  ...
  const buildMap = (jobsArr) => {
    ...
    jobsArr.filter(j => j.lat && j.lng).forEach(j => {
      ...
      L.circleMarker([j.lat, j.lng], { radius:11, fillColor:col, color:"#fff", weight:2.5, opacity:1, fillOpacity:0.9 })
        .addTo(m)
        .bindPopup(`<div style="font-family:system-ui,sans-serif;min-width:160px;padding:4px">
          <b style="font-size:13px">${j.customer}</b><br>
          <span style="color:#666;font-size:12px">${j.jobType} \u00b7 ${j.material}</span>
          ${j.address ? `<br><span style="color:#999;font-size:11px">${j.address}</span>` : ""}
          <br><b style="font-size:15px;display:block;margin:6px 0">${fmt(j.amount)}</b>
          <span style="background:${col};color:#fff;padding:2px 10px;border-radius:99px;font-size:11px;font-weight:600">${STATUSES[j.status]?.label||j.status}</span>
          <br><span style="color:#aaa;font-size:11px;margin-top:4px;display:block">Close: ${j.close}</span>
        </div>`);
    });
  ...
  const loadAndBuildMap = (jobsArr) => {
    ...
    s.onload = () => buildMap(jobsArr);
    s.onerror = () => setMapError("Map library unavailable.");
  ...
  }, [tab, jobs]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const stats = useMemo(() => {
    ...
    return {
      activeCount: active.length, pipeline, revenue, decided, followUp,
  ...
  const calPipeline = useMemo(() => {
    ...
    return jobs.filter(j => {
      const d = new Date(j.close + "T00:00:00");
      return d.getFullYear() === y && d.getMonth() === m && ["quote","open","in_progress"].includes(j.status);
  ...
  const custGroups = useMemo(() => {
    ...
    jobs.forEach(j => {
      ...
      g[j.customer].jobs.push(j);
      g[j.customer].total += j.amount;
  ...
  const displayJobs = useMemo(() => {
    let list = jobs
      ...
      .filter(j => {
        ...
        return !q || j.customer.toLowerCase().includes(q) || j.jobType.toLowerCase().includes(q) || j.material.toLowerCase().includes(q);
      });
    list.sort((a,b) => {
  ...
  const upcoming = useMemo(() =>
    jobs.filter(j => ["quote","open","in_progress"].includes(j.status))
      .map(j => ({ ...j, days: daysUntil(j.close) }))
  ...
  const saveJob = async () => {
    ...
    if (form.address && (!lat || !lng)) {
      try {
        ...
        if (d[0]) { lat = +d[0].lat; lng = +d[0].lon; }
      } catch {}
    }
    ...
    const parsed = {
      ...
      sqft:      parseAmt(form.sqft) || 0,
      lat,
      lng,
      createdAt: form.createdAt || TODAYSTR,
    };

    const payload = jobToRow(parsed);
    const result  = editId
  ...
  const Arr = ({ col }) => (
    <span style={{ opacity:.4, fontSize:10, marginLeft:2 }}>
    ...
    <div className="min-h-screen" style={{ background: G.parch, fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif" }}>
      ...
      <header style={{ background: G.dark }} className="px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shadow-xl">
        ...
        <div className="flex items-center gap-2">
          {stats.followUp.length > 0 && (
            <div className="hidden sm:flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold"
              style={{ background:"rgba(212,168,67,.18)", color:"#d4a843", border:"1px solid rgba(212,168,67,.35)" }}>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-5 py-4 space-y-4">
        ...
        {loadingJobs && (
          <div className="rounded-2xl border px-4 py-3 text-sm"
            style={{ background:"#fff", borderColor:G.border, color:G.muted }}>
            Loading jobs&hellip;
          </div>
        )}
        ...
        {tab === "dashboard" && <>
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm border" style={{ borderColor: G.border }}>
            <div className="flex items-center justify-between px-4 py-3" style={{ background: G.dark }}>
              <button onClick={prevMonth} className="w-8 h-8 flex items-center justify-center rounded-xl text-white text-xl font-black hover:opacity-70">&#8249;</button>
                    ...
                    {cell && (
                      <div className="p-1 sm:p-1.5">
                        ...
                        <div className="mt-0.5 space-y-px">
                          {cell.jobs.slice(0, maxPerDay).map(j => (
                            <div key={j.id}
                              className={`text-xs px-1 sm:px-1.5 py-px sm:py-0.5 rounded font-semibold truncate leading-tight ${STATUSES[j.status]?.calCls || ""}`}
                              style={{ fontSize:"clamp(9px,2.2vw,12px)" }}>
                              {j.customer.split(" ")[0]}
                            </div>
                          ))}
                          {cell.jobs.length > maxPerDay && (
                            <div className="text-xs px-1 font-medium" style={{ color:G.muted }}>
                              +{cell.jobs.length - maxPerDay}
                            </div>
                          )}
                        </div>
                ...
                <div>
                  ...
                  {selJobs.length > 0 && (
                    <p className="text-xs mt-0.5" style={{ color:G.muted }}>
                      {selJobs.length} job{selJobs.length > 1 ? "s" : ""} &middot; {fmt(selJobs.reduce((s,j) => s+j.amount, 0))} total
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <GoldBtn small onClick={() => openNew(selectedDay)}>+ Add Job</GoldBtn>
                  <button onClick={() => setSelectedDay(null)} className="w-7 h-7 flex items-center justify-center rounded-full text-base" style={{ color:G.muted }}>&times;</button>
                </div>
              </div>
              {selJobs.length === 0 && (
                <div className="px-4 py-6 text-center text-sm" style={{ color:G.muted, background:"#fff" }}>
                  No jobs closing on this date. Double-tap or tap &ldquo;+ Add Job&rdquo; to create one.
                </div>
              )}
              {selJobs.map(j => (
                <div key={j.id} className="flex items-center gap-3 px-4 py-3 border-t flex-wrap gap-y-2" style={{ borderColor:G.border, background:"#fff" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color:G.muted }}>{j.jobType} &middot; {j.material}{j.sqft ? ` &middot; ${j.sqft} sf` : ""}</p>
              ...
              {upcoming.map(j => (
                <div key={j.id} className="flex items-center gap-3 py-2.5 border-b last:border-0" style={{ borderColor:G.border }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color:G.muted }}>{j.jobType} &middot; {j.material}</p>
                  </div>
                  <div className="text-right">
                  ...
                  <button onClick={() => jumpDay(j.close)} title="Jump to calendar"
                    className="text-xs w-7 h-7 flex items-center justify-center rounded-lg hover:bg-green-50"
                    style={{ color:G.light }}>&#128197;</button>
                </div>
              ))}
            </div>
                  ...
                  <div key={key} className="flex items-center gap-2 mb-2.5 last:mb-0">
                    ...
                    <div className="flex-1 rounded-full h-4 overflow-hidden" style={{ background:G.border }}>
                      <div className="h-4 rounded-full flex items-center justify-end pr-1.5 transition-all"
                        style={{ width:`${(vals[i]/mx)*100}%`, background:s.hex }}>
          ...
          {stats.followUp.length > 0 && (
            <div className="rounded-2xl p-4 border" style={{ background:"#fef8ee", borderColor:"#e8d090" }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs hidden sm:block" style={{ color:G.muted }}>&mdash; no update in 30+ days</span>
              </div>
        ...
        {tab === "jobs" && <>
          <div className="flex flex-col sm:flex-row gap-2">
            <input type="text" placeholder="Search customer, type, material..." value={search} onChange={e => setSearch(e.target.value)} className={inp + " flex-1"} />
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {displayJobs.length === 0 && (
                    <tr><td colSpan={9} className="text-center py-12 text-sm" style={{ color:G.muted }}>No jobs found.</td></tr>
                  )}
                  {displayJobs.map((j, i) => {
                    ...
                    return (
                      <tr key={j.id} className="border-b transition-colors"
                        style={{ borderColor:G.border, background:i%2===0?"#fff":"#faf8f4", borderLeft:fu?`3px solid ${G.gold}`:"" }}>
                        ...
                        <td className="px-3 py-3 whitespace-nowrap">
                          <div className="flex gap-1.5">
                            ...
                            <button onClick={() => deleteJob(j.id)}
                              ...
                              style={{ borderColor:"#fecaca", color:"#ef4444", background:"#fff9f9" }}>Del</button>
                          </div>
                        </td>
                      </tr>
                    );
            ...
            {custGroups.map(cg => {
              ...
              return (
                <div key={cg.name} className="rounded-2xl overflow-hidden shadow-sm border" style={{ borderColor:G.border }}>
                  <div className="flex items-center justify-between px-4 py-4 cursor-pointer hover:opacity-95 bg-white"
                    onClick={() => setExpandedCust(open ? null : cg.name)}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{ color:G.muted }}>{cg.jobs.length} job{cg.jobs.length !== 1 ? "s" : ""} &middot; {[...new Set(cg.jobs.map(j => j.jobType))].join(", ")}</p>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-xs uppercase" style={{ background:"#faf8f4", color:G.muted }}>
                            {["Type","Material","Status","Amount","$/sf","Close","Notes",""].map((h,i) => (
        ...
        {tab === "map" && <>
          <div className="rounded-2xl p-3 sm:p-4 border shadow-sm flex items-center justify-between flex-wrap gap-3 bg-white" style={{ borderColor:G.border }}>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color:G.muted }}>{jobs.filter(j => j.lat && j.lng).length} of {jobs.length} plotted &middot; Click pin for details</p>
            </div>
      ...
      {showExport && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-3 sm:p-6"
          style={{ background:"rgba(26,60,43,.6)", zIndex:9999, backdropFilter:"blur(4px)" }}>
          <div className="w-full max-w-md rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl" style={{ background:"#fff" }}>
            <div className="px-6 py-5 flex items-center justify-between" style={{ background:G.dark }}>
              ...
              <button onClick={() => setShowExport(false)} className="w-8 h-8 flex items-center justify-center rounded-full text-white text-xl"
                style={{ background:"rgba(255,255,255,.1)" }}>&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2" style={{ color:G.muted }}>Select Month</label>
                <select value={exportMY} onChange={e => setExportMY(e.target.value)} className={inp}>
              ...
              <div className="rounded-2xl p-4 border" style={{ background:G.parch, borderColor:G.border }}>
                ...
                {exportJobs.length === 0
                  ...
                  : <>
                    ...
                    <p className="text-xs mt-0.5" style={{ color:G.muted }}>{exportJobs.length} job{exportJobs.length !== 1 ? "s" : ""} &middot; {monthOptions.find(o => o.val === exportMY)?.label}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-6"
          style={{ background:"rgba(26,60,43,.65)", zIndex:9999, backdropFilter:"blur(6px)" }}>
          <div className="w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl" style={{ maxHeight:"94vh", overflowY:"auto" }}>
            <div className="px-5 sm:px-6 py-4 sm:py-5 flex items-center justify-between sticky top-0 z-10" style={{ background:G.dark }}>
              ...
              <button onClick={closeForm} className="w-8 h-8 flex items-center justify-center rounded-full text-white text-xl"
                style={{ background:"rgba(255,255,255,.1)" }}>&times;</button>
            </div>
            <div className="p-5 sm:p-6 space-y-4" style={{ background:"#fff" }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.jobType} onChange={e => setForm({ ...form, jobType:e.target.value })} className={inp}>
                    {JOB_TYPES.map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color:G.muted }}>Material</label>
                  <select value={form.material} onChange={e => setForm({ ...form, material:e.target.value })} className={inp}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="rounded-xl border px-4 py-2.5 text-sm font-bold" style={{ borderColor:G.border, background:G.parch, color:G.dark }}>
                    {form.sqft && form.amount
                      ? `$${(parseAmt(form.amount)/parseAmt(form.sqft)).toFixed(2)}/sf`
                      : <span className="font-normal" style={{ color:G.muted }}>Auto-calculated</span>
                    }
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color:G.muted }}>
                  Address / City <span className="font-normal normal-case" style={{ color:G.border }}>(for map)</span>
                </label>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm({ ...form, notes:e.target.value })} rows={3}
                  placeholder="Material details, measurements, special requests..." className={inp + " resize-none"} />
              </div>
              <div className="flex gap-2 pt-1">
                ...
                {editId && (
                  <GreenBtn onClick={() => printJobQuote({ ...form, amount:parseAmt(form.amount), sqft:parseAmt(form.sqft)||0 })}>
                    &#128196; PDF
                  </GreenBtn>
                )}
